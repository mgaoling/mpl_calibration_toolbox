...
private:
  ...
  cv::Mat     undist_maps_x_;  // Undistortion Map on X axis
  cv::Mat     undist_maps_y_;  // Undistortion Map on Y axis
...
public:
  ...
  cv::Mat     camera_matrix() { return cam_mtx_; }
  cv::Mat     distortion_coefficients() { return dist_coeff_; }
  cv::Mat     rectification_matrix() { return rect_mtx_; }
  cv::Mat     projection_matrix() { return proj_mtx_; }
  cv::Mat     undistortion_map_x() { return undist_maps_x_; }
  cv::Mat     undistortion_map_y() { return undist_maps_y_; }
};
